...
### L8: 并行工具调用是 Token 节省的基础
- **规则**：无依赖的工具调用必须在同一轮并行发出
- **收益**：减少轮次 → 减少上下文膨胀 → 省 Token
- **案例**：搜索+读文件+检查状态 → 一轮完成（vs 三轮）

### L9: LCM 早压缩比晚压缩更有效
- **数据**：20% 压缩（200K）vs 60% 压缩（600K）
- **效果**：早压缩保持更多有效上下文，减少幻觉
- **建议**：MiMo 1M 窗口下，20%（200K）是最佳压缩点

### L10: 工具选择直接影响执行速度
- **规则**：能用专用工具就不用 terminal
- **案例**：read_file（<1s）vs cat（1-2s）vs browser（>10s）
- **收益**：每个工具调用省 0.5-1 秒，累积效果显著


### L11: 优化策略有明确的实施优先级
- **立即可做**（0 成本）：系统提示稳定、工具定义固定、并行调用、专用工具
- **已实施**（持续生效）：辅助模型分层、LCM 早压缩、工具搜索、超时控制、no_agent
- **计划实施**：结构化输出、请求批处理、成本监控、Session-sticky
...
### L13: 实战验证是优化闭环的关键
- **理论**：所有优化策略都需要实际数据验证
- **实践**：每日验证缓存命中率+Token消耗+成本
- **调整**：根据验证结果动态调整策略

### L14: 网页研究是 Token 消耗最大的场景
- **数据**：browser 方式 75,000 Token vs web_search+extract 2,500 Token
- **节省**：97%
- **规则**：信息搜索用 web_search，内容提取用 web_extract，交互操作才用 browser

### L15: 监控任务必须用 no_agent 模式
- **数据**：agent 模式 144,000 Token/天 vs no_agent 0 Token/天
- **节省**：100%
- **规则**：数据采集、参数巡检、价格追踪、定时报告 → no_agent


## 飞轮库结构统一 (2026-06-20)

### L13: 双目录结构（wiki/ + concepts/）是技术债
- **现象**：wiki/ 只有 9 篇，concepts/ 有 54 篇，hot.md 虚报总数
- **根因**：早期用 concepts/ 存研究笔记，后来改用 wiki/，未迁移
- **修复**：全量迁入 wiki/，concepts/ 保留但不再新增
- **教训**：飞轮库应统一用 wiki/ 一个目录，concepts/ 是遗留结构

### L14: BOOTSTRAP.md 必须跟环境同步
- **现象**：BOOTSTRAP.md 仍是 Windows 路径（D:/、macOS），Linux 不兼容
- **根因**：从 Windows 迁移到 Linux 时未更新
- **修复**：重写为 v2.0，Linux 路径 + MiMo 模型 + 当前规则
- **教训**：环境变更时必须同步 BOOTSTRAP.md

### L15: index.md 是飞轮库的导航核心
- **现象**：index.md 引用不存在的文件，分类过时
- **根因**：多次迭代后未同步更新
- **修复**：重建为 14 个分类，63 篇全部索引
- **教训**：每次新增 Wiki 必须同步更新 index.md


### L16: CC 不需要读整个飞轮库
- **数据**：飞轮库 63 篇 = 50K+ Token，90% 是 Hermes 的内容
- **正确做法**：提炼 CC 需要的 5% 写 CLAUDE.md（104 行 = 3K Token）
- **效果**：CC 每次启动自动注入，零额外调用消耗
- **教训**：给 CC 增强能力用 CLAUDE.md，不用飞轮库


### L17: CC 的 CLAUDE.md 有层级结构
- **全局**：`~/.claude/CLAUDE.md`（每次启动自动加载）
- **项目级**：项目根目录/CLAUDE.md（进入项目时加载）
- **子目录级**：任意子目录/CLAUDE.md（在该目录工作时自动加载）
- **个人级**：local.claude.md（不提交 Git）
- **优先级**：子目录 > 项目级 > 全局

### L18: CC 三阶段工作流与 NΞXUS 四阶段对应
- 先问答熟悉代码 → 阶段 0：需求理解
- 先规划后执行 → 阶段 1：拆解分发
- 执行+工具验证 → 阶段 2：执行验证
- 迭代优化 → 阶段 3：终验报告
- **结论**：官方建议与 NΞXUS 防作弊流程完全吻合

### L19: CC 支持自然语言 Git 指令
- "提交推送 PR" → 自动完成 commit + push + 创建 PR
- 不需要记住具体 git 命令
- **实践**：Hermes 给 CC 的任务描述可以用自然语言

### L20: 用户不精确是因为不知道，不是偷懒

- **旧认知**：用户说模糊 → 按自己的理解执行
- **新认知**：用户说模糊 → 主动问对问题，帮用户理清思路
- **机制**：多轮提问（2-3 个/轮），逐步收敛
- **效果**：方案更精准，减少返工

### L21: 对结果负责，不是对执行动作负责

- **旧模式**：执行完 → 报告 → 结束
- **新模式**：执行前确认方案 → 执行中验证产出 → 执行后跟踪效果
- **关键**：发现没达预期 → 分析原因 → 主动调整方案 → 继续跟踪
- **教训**：我负责的是"达成目标"，不是"做完任务"
